...
public class Group {
    ...
    public void completeTask(){
        ...
        int pos = EntityManager.entities.get(leader).getPosition();
        // eating goal
        if(task == 1) {
            InteractableObject obj;
            // some setup
            if(pos == -1)
                obj= World.tileMap.get(EntityManager.entities.get(leader).getCurrentTask().getTargetTile()[0]).
                    get(EntityManager.entities.get(leader).getCurrentTask().getTargetTile()[1]).
                    getObjectsInTile().get(EntityManager.entities.get(leader).getCurrentTask().getObjectID());
            else
                obj = Objects.requireNonNull(World.getMap(pos)).getTileMap().get(EntityManager.entities.get(leader).
                        getCurrentTask().getTargetTile()[0]).get(EntityManager.entities.get(leader).getCurrentTask()
                        .getTargetTile()[1]).getObjectsInTile().get(EntityManager.entities.get(leader).getCurrentTask()
                        .getObjectID());
            //
            for(int id : entitiesInGroup){
